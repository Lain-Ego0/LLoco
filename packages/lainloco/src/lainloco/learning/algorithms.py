"""Reusable custom algorithms backed by RSL-RL's PPO."""
...
class Go2AuxiliaryPPO(PPO):
  ...
  student_distill_optimizer: torch.optim.Optimizer

  def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    if (
      self.auxiliary_kind == "cts"
      and isinstance(self.actor, CtsActorModel)
      and isinstance(self.critic, CtsCriticModel)
    ):
      ...
      self.optimizer = type(self.optimizer)(
        unique_parameters, **self.optimizer.defaults
      )
    elif self.auxiliary_kind == "dreamwaq" and isinstance(
      self.actor, DreamWaQActorModel
    ):
      ...
      self.optimizer = type(self.optimizer)(ppo_parameters, **self.optimizer.defaults)
  ...
  def _load_ts_teacher_encoders(self, auxiliary: TeacherStudentActorCritic) -> bool:
    ...
    loaded_groups = {key.split(".", 1)[0] for key in candidate if "." in key}
  ...
  def process_env_step(self, obs, rewards, dones, extras):
    ...
    if (
      self.uses_amp
      and hasattr(self, "amp_auxiliary")
      and hasattr(self, "amp_normalizer")
      and self.transition.observations is not None
    ):
  ...
  def _build_auxiliary(self) -> None:
    ...
    if self.auxiliary_kind == "cts" and history is not None and privileged is not None:
      ...
      if critic is not None:
        ...
        if isinstance(self.actor, CtsActorModel):
      ...
      if critic is not None:
        ...
        if isinstance(self.actor, DreamWaQActorModel):
          self.auxiliary.vae = self.actor.vae
    elif (
      self.auxiliary_kind == "ts"
      ...
      if critic is not None:
        ...
        if isinstance(self.actor, StudentActorModel):
          self.auxiliary.student_lstm = self.actor.student_lstm
          self.auxiliary.student_head = self.actor.student_head
          self._ts_teacher_checkpoint_loaded = self._load_ts_teacher_encoders(
            self.auxiliary
          )
        ...
        elif isinstance(self.actor, TeacherActorModel):
          self.auxiliary.terrain_encoder = self.actor.terrain_encoder
          self.auxiliary.privileged_encoder = self.actor.privileged_encoder
    if self.auxiliary is not None:
      # Source CTS student encoders and DreamWaQ VAE both use 1e-3.
      auxiliary_parameters = (
        self.actor.vae.parameters()
        if self.auxiliary_kind == "dreamwaq"
        and isinstance(self.actor, DreamWaQActorModel)
        else self.auxiliary.parameters()
      )
      self.auxiliary_optimizer = torch.optim.Adam(auxiliary_parameters, lr=1e-3)

  def _optimize_auxiliary(
    self,
    sample_count: int,
    key: str,
    loss_fn,
    ...
    if sample_count == 0:
      return {key: 0.0}
    assert self.auxiliary_optimizer is not None
    ...
    for _ in range(self.num_learning_epochs):
      for indices in torch.randperm(sample_count, device=self.device).tensor_split(
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(clipped_parameters, 1.0)
  ...
  def _auxiliary_update(self) -> dict[str, float]:
    ...
    if self.auxiliary_kind == "cts" and isinstance(auxiliary, CtsActorCritic):
      ...
      def loss_fn(indices):
        return auxiliary.distillation_loss(privileged[indices], history[indices])

      return self._optimize_auxiliary(
        history.shape[0], "cts_distillation", loss_fn, auxiliary.parameters()
      )

    if self.auxiliary_kind == "dreamwaq" and isinstance(auxiliary, DreamWaQActorCritic):
      ...
      def loss_fn(indices):
        target_velocity = None if explicit is None else explicit[indices]
        return auxiliary.auxiliary_loss(
          history[indices],
      ...
      return self._optimize_auxiliary(
        history.shape[0], "dreamwaq_auxiliary", loss_fn, auxiliary.parameters()
      )

    if self.auxiliary_kind == "ts" and isinstance(auxiliary, TeacherStudentActorCritic):
      assert history is not None and privileged is not None and terrain is not None

      def loss_fn(indices):
        return auxiliary.distillation_loss(
          terrain[indices], privileged[indices], history[indices]
        )

      return self._optimize_auxiliary(
        history.shape[0], "ts_distillation", loss_fn, auxiliary.parameters()
      )
    return {}
  ...
  def save(self) -> dict:
    ...
    if self.auxiliary is not None:
      ...
      if self.auxiliary_optimizer is not None:
        saved["go2_auxiliary_optimizer_state_dict"] = (
          self.auxiliary_optimizer.state_dict()
        )
  ...
  def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
    ...
    if self.auxiliary is not None and "go2_auxiliary_state_dict" in loaded_dict:
      self.auxiliary.load_state_dict(
        loaded_dict["go2_auxiliary_state_dict"], strict=strict
      )
      if (
        self.auxiliary_optimizer is not None
        and "go2_auxiliary_optimizer_state_dict" in loaded_dict
      ):
        self.auxiliary_optimizer.load_state_dict(
          loaded_dict["go2_auxiliary_optimizer_state_dict"]
        )
    ...
    if "go2_amp_discriminator_state_dict" in loaded_dict:
      if not hasattr(self, "amp_auxiliary"):
        self._amp_update()
      self.amp_auxiliary.load_state_dict(
        loaded_dict["go2_amp_discriminator_state_dict"], strict=strict
      )
      ...
      if "go2_amp_normalizer_state_dict" in loaded_dict:
        self.amp_normalizer.load_state_dict(
          loaded_dict["go2_amp_normalizer_state_dict"]
        )
