# RoboLab Phase 0–1 开发主线与批次计划（归档）

状态：历史归档。v0.1 于 2026-08-20 建立，B1–B6 已完成，B7 转入下一阶段
N0 关闭门禁继续跟踪。当前进度请查阅
[`../project/DEVELOPMENT_PLAN.md`](../project/DEVELOPMENT_PLAN.md)。

历史分工：[`../project/ROADMAP.md`](../project/ROADMAP.md) 管阶段与退出条件，
[`../project/MVP_ACCEPTANCE.md`](../project/MVP_ACCEPTANCE.md) 管 MVP 验收，本文曾负责
B1–B7 的批次顺序与追溯。本文不再作为当前进度依据；设计取舍见
[`DECISIONS_AND_QA.md`](DECISIONS_AND_QA.md)。
...
| B7.3 | 本地环境测试记录：MJLab play / Viser 手动验证清单 | D-030 | §3 | 🔶 2026-08-21（Viser 已启动；`--video` 与 PlayConfig 缺陷已修复；trained play 仍需真实 checkpoint/WandB run，详见 `../project/B7_ACCEPTANCE_RECORD.md`） |

依赖：B4、B6。

## 维护规则

1. 以下规则是 Phase 0–1 执行期间使用的历史维护规则；本文现已只读归档；
