...
状态：R0、R1、R2 已完成，当前 `ACTIVE_STAGE: R3`。本文是阶段状态、依赖关系和执行入口的唯一权威来源。
...
- R2 的唯一参考机器人是 FireDog 2.2；R3 在此基础上实施真实 simulation-first 策略工作流。

## 当前阶段

R3 是当前激活阶段，目标是为 FireDog 2.2 完成真实 velocity 策略的 train/play/evaluate/export
和固定 PolicyArtifact。R4–R6 只用于理解后续目标，不得提前实现阶段性大功能。
...
6. 当前 GPU 被占用只阻止需要 GPU 的验收，不阻止 R3 的 CPU 配置、schema 和 contract 工作；真实训练仍必须使用 GPU 完成。
