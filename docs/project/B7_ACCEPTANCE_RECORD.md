...
状态：历史 Unitree/MJLab 1.2 验收记录；创建日期 2026-08-20，2026-08-21 被 MJLab 1.6 主线计划取代。

本文件保留已经发生的 CPU 与手动验证事实，不改写历史证据。文中的环境、安装、发现和 play 命令均已失效，
不得在当前工作树执行或作为当前安装指导；原 B7/N0 剩余工作不再作为可执行兼容任务，也不再阻塞 MJLab 1.6
定制、自研机器人接入、平台整合或 Runtime。
当前状态和批次只以 [`DEVELOPMENT_PLAN.md`](DEVELOPMENT_PLAN.md) 为准；本文后续要求更新 N0 状态的文字不再适用。

历史执行顺序为 **B7.1 → B7.2 → B7.3**。旧 N0.2–N0.7 映射和双后端迁移门禁已经废止；
Unitree sim-to-sim 仍可作为 legacy 兼容证据。本文只记录证据，不重新定义当前批次、依赖或完成状态。
