...
class DreamActor:
  ...
  def __call__(
    ...
    self._current.copy_(_noise(frame, joint_position_noise) if add_noise else frame)
...
class DreamCriticHistory:
  ...
  def __call__(self, env, command_name: str, sensor_name: str) -> torch.Tensor:
    ...
    heights = (
      torch.clamp(scan.frame_pos_w[:, 0, 2:3] - 0.5 - scan.hit_pos_w[..., 2], -1.0, 1.0)
      * 5.0
    )
