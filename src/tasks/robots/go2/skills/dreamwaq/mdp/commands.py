...
class DreamWaQVelocityCommand(UniformVelocityCommand):
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    moving = torch.linalg.vector_norm(self.vel_command_b[env_ids, :2], dim=1) > 0.2
  ...
  def _update_command(self, env_ids=None) -> None:
    ...
    high = torch.arange(self.num_envs, device=self.device) < int(self.num_envs * 0.2)
...
def source_command_curriculum(env, env_ids, command_name: str, max_curriculum: float):
  ...
  if env.common_step_counter and env.common_step_counter % max_steps == 0:
    ...
    if (
      len(high)
      and len(low)
      and torch.mean(reward[high]) / max_steps > 0.8 * weight
      and torch.mean(reward[low]) / max_steps > 0.8 * weight
    ):
  ...
  term = env.command_manager.get_term(command_name)
  return {
    "lin_vel_x_max": torch.tensor(term.cfg.ranges.lin_vel_x[1], device=env.device)
  }
