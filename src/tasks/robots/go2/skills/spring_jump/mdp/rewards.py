...
def line_z(env, command_name, sensor_name):
  s = _state(env)
  s.update(env, command_name, sensor_name)
  r = env.scene["robot"]
...
def base_height_flight(env, command_name, sensor_name):
  ...
  return torch.exp(-torch.abs(z - 0.47) * 5) * s.was_in_flight * ~s.has_jumped * 6
...
def base_height_stance(env, command_name, sensor_name):
  ...
  s.update(env, command_name, sensor_name)
  z = env.scene["robot"].data.root_link_pos_w[:, 2]
  return torch.exp(-torch.abs(z - 0.35) * 5) * s.has_jumped * (z > 0.2)
...
def land_pos(env, command_name, sensor_name):
  ...
  r = env.scene["robot"]
  return (
    torch.exp(-torch.abs(s.start_pos + c[:, :2] - s.landing_pos).sum(1))
    * s.has_jumped
    * (root_euler(r).sum(1) < 0.6)
    * (s.max_height > 0.42)
  )
...
def dof_pos(env):
  r = env.scene["robot"]
  ids = joint_ids(r)
  return torch.abs(r.data.joint_pos[:, ids] - r.data.default_joint_pos[:, ids]).sum(1)


def hip_pos(env):
  r = env.scene["robot"]
  ids = joint_ids(r)
  return torch.abs(r.data.joint_pos[:, ids] - r.data.default_joint_pos[:, ids])[
    :, (0, 3, 6, 9)
  ].sum(1)
...
def tracking_lin_vel(env, command_name, sensor_name):
  ...
  s.update(env, command_name, sensor_name)
  c = env.command_manager.get_command(command_name)
  v = env.scene["robot"].data.root_link_lin_vel_b[:, 0]
  return (
    torch.exp(-torch.square(c[:, 0] * 1.6 - v)) * s.was_in_flight * ~s.has_jumped * 5
  )
...
def line_vel_stance(env, command_name, sensor_name):
  ...
  s.update(env, command_name, sensor_name)
  return (
    torch.abs(env.scene["robot"].data.root_link_lin_vel_b[:, :2]).sum(1) * s.has_jumped
  )
...
def dof_pos_limits(env):
  r = env.scene["robot"]
  ids = joint_ids(r)
  limits = r.data.soft_joint_pos_limits
  assert limits is not None
  pos = r.data.joint_pos[:, ids]
  return (
    -(pos - limits[:, ids, 0]).clamp(max=0.0) + (pos - limits[:, ids, 1]).clamp(min=0.0)
  ).sum(1)
...
def feet_contact_forces(env, sensor_name, max_contact_force: float):
  ...
  return (
    (torch.linalg.vector_norm(force, dim=-1) - max_contact_force).clamp(min=0.0).sum(1)
  )
...
def foot_clearance(env, command_name, sensor_name):
  ...
  height = quat_apply_inverse(quat.reshape(-1, 4), root_to_foot.reshape(-1, 3)).reshape(
    -1, len(ids), 3
  )[..., 2]
  return torch.abs(height + 0.20).sum(1) * s.was_in_flight * ~s.has_jumped * 6
