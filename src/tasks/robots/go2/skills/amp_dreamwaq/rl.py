...
class _AmpReplayBuffer:
  ...
  def insert(self, states: torch.Tensor, next_states: torch.Tensor) -> None:
    if states.shape != next_states.shape or states.shape[1] != 31:
      raise ValueError(
        f"Invalid AMP transition shapes: {states.shape}, {next_states.shape}"
      )
  ...
  def __init__(
    ...
    **kwargs,
    ...
    if self.rnd is not None or self.symmetry is not None or self.is_multi_gpu:
      raise ValueError(
        "AMP-DreamWaQ source parity does not support RND, symmetry, or multi-GPU"
      )
    device = torch.device(self.device)
    self.amp_reward_coef = amp_reward_coef
    self.discriminator = _Discriminator(device, tuple(amp_discr_hidden_dims))
    self.motion = Go2AmpMotionLoader(
      device, dt=0.02, preload=amp_num_preload_transitions
    )
  ...
  def process_env_step(self, obs, rewards, dones, extras):
    ...
    with torch.no_grad():
      ...
      amp_reward = (
        ...
        * torch.clamp(1.0 - 0.25 * torch.square(prediction - 1.0), min=0.0)
      ).squeeze(1)
  ...
  def update(self) -> dict[str, float]:
    ...
    for batch in generator:
      ...
      amp_loss = 0.5 * (
        ...
        + torch.nn.functional.mse_loss(
          policy_prediction, -torch.ones_like(policy_prediction)
        )
      )
      gradient_penalty = self.discriminator.gradient_penalty(expert_state, expert_next)
      loss = (
        surrogate_loss
      ...
      values_to_add = {
        ...
        "vae": vae_loss,
        "velocity_estimation": velocity_loss,
        "reconstruction": reconstruction_loss,
        "vae_kl": kl_loss,
        "amp": amp_loss,
        "amp_grad_penalty": gradient_penalty,
