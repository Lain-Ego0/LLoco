"""Rollout storage shared by LainLoco custom algorithms."""
...
def _tensor_state_value(
  state: Mapping[str, torch.Tensor | float | int], key: str
) -> torch.Tensor:
  value = state[key]
  if not isinstance(value, torch.Tensor):
    raise TypeError(f"Checkpoint field {key!r} must be a tensor")
  return value
...
class Go2RunningNormalizer:
  ...
  def load_state_dict(self, state: dict[str, torch.Tensor | float]) -> None:
    self.mean.copy_(_tensor_state_value(state, "mean"))
    self.var.copy_(_tensor_state_value(state, "var"))
    self.count.copy_(_tensor_state_value(state, "count"))
...
class Go2RolloutStorage:
  ...
  def compute_returns(
    ...
    self.advantages = (returns - values - (returns - values).mean()) / (
      (returns - values).std() + 1e-8
    )
  ...
  def batches(self, batch_size: int):
    ...
    flat = [
      torch.stack([getattr(item, field) for item in self._items]).flatten(0, 1)
      for field in ("obs", "critic_obs", "actions", "log_prob")
    ]
    for start in range(0, len(indices), batch_size):
      idx = indices[start : start + batch_size]
      yield tuple(t[idx] for t in flat) + (
        self.returns.flatten(0, 1)[idx],
        self.advantages.flatten(0, 1)[idx],
      )
...
class Go2AmpReplayBuffer:
  ...
  def load_state_dict(self, state: dict[str, torch.Tensor | int]) -> None:
    self.states.copy_(_tensor_state_value(state, "states"))
    self.next_states.copy_(_tensor_state_value(state, "next_states"))
