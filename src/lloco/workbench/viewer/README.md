# LainLab Model Inspector

LainLab 的精简模型检查页面，由本项目编写。参考 robot_viewer 的检查项划分思路，未沿用其应用源码或 UI。
...
`npm ci && npm run build` 输出到 `../static/viewer/`。运行依赖 LainLab 工作台的模型 API，而非额外查看器进程。

Three.js 使用 MIT 许可证，见 `public/THREE_LICENSE.txt`。LainLab 自编代码遵循项目根许可证。
