...
def _observations(cfg: ManagerBasedRlEnvCfg, profile: SpringJumpProfile) -> None:
  cfg.observations = {
    "actor": ObservationGroupCfg(
      terms={
        "history": ObservationTermCfg(
          func=mdp.observations.SpringActorHistory,
    ...
    "critic": ObservationGroupCfg(
      terms={
        "history": ObservationTermCfg(
          func=mdp.observations.SpringCriticHistory,
...
def _rewards(cfg: ManagerBasedRlEnvCfg) -> None:
  ...
  cfg.rewards = {
    "before_setting": RewardTermCfg(
      func=mdp.rewards.BeforeSetting, weight=5.0, params=state
    ),
    ...
    "base_height_flight": RewardTermCfg(
      func=mdp.rewards.base_height_flight, weight=3.0, params=state
    ),
    "base_height_stance": RewardTermCfg(
      func=mdp.rewards.base_height_stance, weight=-10.0, params=state
    ),
    ...
    "dof_vel_limits": RewardTermCfg(
      func=lambda env: (
        (torch.abs(env.scene["robot"].data.joint_vel[:, :12]) - 30.0)
        .clamp(min=0.0)
        .sum(1)
      ),
      weight=-1.0,
    ...
    "collision": RewardTermCfg(
      func=mdp.rewards.collision, weight=-50.0, params={"sensor_name": PENALIZED_SENSOR}
    ),
    ...
    "tracking_lin_vel": RewardTermCfg(
      func=mdp.rewards.tracking_lin_vel, weight=5.0, params=state
    ),
    "line_vel_stance": RewardTermCfg(
      func=mdp.rewards.line_vel_stance, weight=-3.0, params=state
    ),
    ...
    "foot_clearance": RewardTermCfg(
      func=mdp.rewards.foot_clearance, weight=-3.0, params=state
    ),
  }
...
def make_spring_jump_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  cfg.commands = {
    "jump_target": mdp.commands.SpringJumpCommandCfg(
      ...
      ranges=UniformVelocityCommandCfg.Ranges(
        lin_vel_x=(0.8, 1.2), lin_vel_y=(0.0, 0.0), ang_vel_z=(0.0, 0.0), heading=None
      ),
    )
  }
  _observations(cfg, profile)
  _rewards(cfg)
  _trot_events(cfg)
  ...
  cfg.events["friction"] = EventTermCfg(
    ...
    mode="startup",
    params={"low": 0.3, "high": 1.0, "num_buckets": 64, "entity_name": "robot"},
  )
  cfg.events["base_mass"].params["ranges"] = (-1.0, 1.0)
  cfg.terminations = {
    "time_out": TerminationTermCfg(func=env_mdp.time_out, time_out=True),
    "base_contact": TerminationTermCfg(
      func=mdp.terminations.below_reset_height, params={"reset_height": 0.15}
    ),
    "base_collision": TerminationTermCfg(
      func=__import__(
        "lloco.tasks.robots.go2.skills.shared.terminations", fromlist=["base_contact"]
      ).base_contact,
...
def make_spring_jump_runner_cfg():
  cfg = make_ppo_runner_cfg(
    SPRING_JUMP.experiment_name,
    max_iterations=50_000,
    save_interval=100,
    symmetry_cfg={
      "data_augmentation_func": "lloco.tasks.robots.go2.skills.spring_jump.mdp.symmetry:spring_jump_symmetry",
