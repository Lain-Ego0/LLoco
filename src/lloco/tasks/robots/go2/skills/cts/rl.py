...
class CtsStudentPolicy(nn.Module):
  ...
  def train_outputs(self, obs, teacher, critic_obs, history, teacher_mode):
    ...
    latent = torch.where(teacher_mode[:, None], teacher_latent, student_latent.detach())
  ...
  def update(self):
    ...
    for _ in range(self.epochs):
      for ids in torch.randperm(count, device=self.device).split(
        ...
        batch_mode = mode[ids]
        surrogate = (
          surrogate_terms[batch_mode].mean() + surrogate_terms[~batch_mode].mean()
        )
        ...
        if self.schedule == "adaptive" and self.desired_kl is not None:
          with torch.no_grad():
            kl = (
              torch.log(dist.scale / old_std[ids])
              + (old_std[ids].square() + (old_mean[ids] - dist.loc).square())
              / (2.0 * dist.scale.square())
              - 0.5
            )
    ...
    for _ in range(self.epochs):
      for ids in torch.randperm(student_ids.numel(), device=self.device).split(
        ...
        student_latent = self.model._normalize_latent(
          self.model.student_encoder(history[batch])
        )
        with torch.no_grad():
          teacher_latent = self.model._normalize_latent(
            self.model.teacher_encoder(teacher[batch])
          )
