...
from lloco.tasks.velocity import make_rough_env_cfg
    ...
    "critic": ObservationGroupCfg(
      terms={
        "history": ObservationTermCfg(
          func=mdp.observations.DreamCriticHistory,
...
def _events(cfg: ManagerBasedRlEnvCfg) -> None:
  ...
  cfg.events = {
    ...
    "reset_robot_joints": EventTermCfg(
      func=shared_events.reset_joints_by_scale,
      mode="reset",
      params={"scale_range": (0.5, 1.5)},
    ),
    ...
    "friction": EventTermCfg(
      ...
      mode="startup",
      params={"low": 0.2, "high": 1.25, "num_buckets": 64},
    ),
    "restitution": EventTermCfg(
      func=shared_events.sample_restitution_label,
      mode="startup",
      params={"low": 0.0, "high": 0.5, "attribute_name": "_dreamwaq_restitution"},
    ),
    ...
    "pd_labels": EventTermCfg(
      func=shared_events.dreamwaq_pd_labels, mode="startup", params={}
    ),
    "motor_zero_offset": EventTermCfg(
      func=dr.encoder_bias, mode="startup", params={"bias_range": (-0.035, 0.035)}
    ),
  }
...
def _rewards(cfg: ManagerBasedRlEnvCfg) -> None:
  cfg.rewards = {
    ...
    "orientation": RewardTermCfg(func=mdp.rewards.orientation, weight=-0.2),
    "base_height": RewardTermCfg(
      func=mdp.rewards.base_height, weight=-5.0, params={"target_height": 0.40}
    ),
    ...
    "collision": RewardTermCfg(
      func=mdp.rewards.collision, weight=-1.0, params={"sensor_name": PENALIZED_SENSOR}
    ),
    ...
    "action_smoothness": RewardTermCfg(
      func=mdp.rewards.action_smoothness, weight=-0.01
    ),
    "stumble": RewardTermCfg(
      func=mdp.rewards.stumble, weight=-0.5, params={"sensor_name": FEET_SENSOR}
    ),
    "foot_clearance": RewardTermCfg(
      func=mdp.rewards.foot_clearance, weight=-0.5, params={"sensor_name": FEET_SENSOR}
    ),
  }
...
def make_dreamwaq_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  cfg.commands = {
    "twist": mdp.commands.DreamWaQVelocityCommandCfg(
      entity_name="robot",
  ...
  cfg.curriculum = {
    "command_velocity": CurriculumTermCfg(
      func=mdp.commands.source_command_curriculum,
...
def make_dreamwaq_runner_cfg():
  cfg = make_ppo_runner_cfg(
    DREAMWAQ.experiment_name, max_iterations=20_000, save_interval=500
  )
  cfg.seed = 1
  cfg.clip_actions = 100.0
  cfg.actor.obs_normalization = False
  cfg.critic.obs_normalization = False
  cfg.actor.class_name = "lloco.tasks.robots.go2.skills.dreamwaq.mdp.rl:DreamWaQActor"
  cfg.algorithm.class_name = "lloco.tasks.robots.go2.skills.dreamwaq.mdp.rl:DreamWaQPPO"
  return cfg
