...
class _VAE(torch.nn.Module):
  ...
  def __init__(self) -> None:
    ...
    self.encoder = torch.nn.Sequential(
      torch.nn.Linear(225, 128),
      torch.nn.ELU(),
      torch.nn.Linear(128, 64),
      torch.nn.ELU(),
    )
    self.mean_latent = torch.nn.Linear(64, 16)
    self.logvar_latent = torch.nn.Sequential(
      torch.nn.Linear(64, 16), torch.nn.Hardtanh(-5.0, 5.0)
    )
    self.mean_vel = torch.nn.Linear(64, 3)
    self.logvar_vel = torch.nn.Sequential(
      torch.nn.Linear(64, 3), torch.nn.Hardtanh(-5.0, 5.0)
    )
  ...
  def forward(
    ...
    if sample:
      latent = mean_latent + torch.randn_like(mean_latent) * torch.exp(
        0.5 * logvar_latent
      )
    ...
    return (
      torch.cat((velocity, latent), dim=-1),
      velocity,
      self.decoder(torch.cat((velocity, latent), dim=-1)),
      mean_latent,
      logvar_latent,
    )
...
class DreamWaQPPO(PPO):
  ...
  def __init__(
    self, *args, vae_learning_rate: float = 1e-3, vae_kl_weight: float = 1.0, **kwargs
  ) -> None:
    super().__init__(*args, **kwargs)
    ...
    self.vae_optimizer = torch.optim.Adam(
      self.actor.vae.parameters(), lr=vae_learning_rate
  ...
  def update(self) -> dict[str, float]:
    ...
    for batch in self.storage.mini_batch_generator(
      ...
      _, estimated_velocity, decoded, mean_latent, logvar_latent = self.actor.vae(
        history, sample=True
      )  # type: ignore[attr-defined]
