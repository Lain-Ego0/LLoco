...
class AmpDreamWaQVelocityCommand(DreamWaQVelocityCommand):
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    high = env_ids < int(self.num_envs * 0.2)
    high_ids = env_ids[high]
    self.vel_command_b[high_ids, 0] = random[: len(high_ids)].uniform_(
      *self.cfg.ranges.lin_vel_x
    )
    self.vel_command_b[high_ids, 1] *= torch.abs(self.vel_command_b[high_ids, 0]) < 1.0

    standing = torch.rand(len(env_ids), device=self.device) < 0.05
    self.vel_command_b[env_ids[standing]] = 0.0
    turning = torch.rand(len(env_ids), device=self.device) < 0.05
