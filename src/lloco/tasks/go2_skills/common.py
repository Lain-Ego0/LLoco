...
from .profiles import JUMP, TROT, JumpProfile, TrotProfile

_GO2_VELOCITY_PROFILE = next(profile for profile in PROFILES if profile.task_name == "Go2")
...
def _trot_rewards(cfg: ManagerBasedRlEnvCfg, profile: TrotProfile) -> None:
  ...
  cfg.rewards = {
    ...
    "dof_acc": RewardTermCfg(
      func=mdp.rewards.DofAcceleration, weight=-2.5e-7
    ),
    ...
    "default_hip_pos": RewardTermCfg(
      func=mdp.rewards.default_hip_pos, weight=-0.2
    ),
...
def _trot_events(cfg: ManagerBasedRlEnvCfg) -> None:
  ...
  cfg.events = {
    ...
    "link_mass": EventTermCfg(
      ...
      params={
        "asset_cfg": SceneEntityCfg(
          "robot", body_names=(r"(FL|FR|RL|RR)_.*",)
        ),
        "ranges": (0.9, 1.1),
        "operation": "scale",
      },
...
def _jump_rewards(cfg: ManagerBasedRlEnvCfg, profile: JumpProfile) -> None:
  cfg.rewards = {
    ...
    "dof_acc": RewardTermCfg(
      func=mdp.rewards.JointVelocityDifference, weight=-5.5e-4
    ),
    "base_height": RewardTermCfg(
    ...
    "default_pos": RewardTermCfg(func=mdp.rewards.default_pos, weight=-0.1),
    "default_hip_pos": RewardTermCfg(
      func=mdp.rewards.jump_default_hip_pos, weight=0.3
    ),
