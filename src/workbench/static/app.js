...
const saved = (() => {
  try {
    return JSON.parse(localStorage.getItem("src.workbench") || "{}");
  } catch {
    return {};
  }
})();
...
window.addEventListener("message", (event) => {
  if (
    ...
    return;
  if (event.data.type === "src:model-loaded")
    notice("模型已加载：" + event.data.path);
  if (event.data.type === "src:model-error") notice(event.data.message, true);
});
document
  .querySelectorAll("[data-step]")
...
$("loadModel").onclick = () => {
  ...
  $("robotFrame").contentWindow.postMessage(
    { type: "src:model", path: $("modelPath").value },
    location.origin,
  );
  notice("正在加载模型和网格…");
  persist();
...
function openMotion(id) {
  ...
  localStorage.setItem("src.motionPreview", id);
  $("motionFrame").hidden = false;
  $("motionFrame").src = "/viewer/?motion=" + encodeURIComponent(id);
}
$("openMotion").onclick = () =>
  openMotion(localStorage.getItem("src.motionPreview"));
window.addEventListener("message", (event) => {
  if (
    event.origin !== location.origin ||
    event.source !== $("motionFrame").contentWindow ||
    event.data?.type !== "src:motion-state"
  )
    return;
  const m = event.data;
...
function motionAssets() {
  ...
  try {
    const pending = JSON.parse(
      localStorage.getItem("src.pendingMotion") || "null",
    );
    if (pending && initialized) {
      ...
      if (job && !job.running) {
        ...
        localStorage.removeItem("src.pendingMotion");
      }
    }
  } catch {
    localStorage.removeItem("src.pendingMotion");
...
function trackCsv(id) {
  localStorage.setItem("src.csvProgress", id);
...
if (localStorage.getItem("src.csvProgress"))
  trackCsv(localStorage.getItem("src.csvProgress"));
