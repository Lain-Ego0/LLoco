...
RoboLab's root MIT license does not replace the license or notices attached to third-party source code, assets, models, data, or documentation. The future `mjlab_native` backend will continue to use the upstream open-source MJLab/MuJoCo stack while keeping RoboLab-owned task and platform code under the root MIT license. Acknowledgement details and the release checklist are maintained in `docs/reference/UPSTREAM_AND_ACKNOWLEDGEMENTS.md`.
