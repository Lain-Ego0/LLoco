...
def health(data_dir: str | Path) -> dict[str, object]:
    ...
    mjlab_import = {"ok": False, "status": "not_implemented", "detail": "MJLab 1.6 toolchain health probe 未在 R0/R1 完成"}
    try:
        import mjlab

        mjlab_import.update({"ok": True, "version": getattr(mjlab, "__version__", "unknown")})
