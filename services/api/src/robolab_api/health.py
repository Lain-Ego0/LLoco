...
def health(data_dir: str | Path) -> dict[str, object]:
    ...
    mjlab_import = {"ok": False, "importOk": False, "status": "not_implemented", "detail": "MJLab 1.6 toolchain health probe 未在 R0/R1 完成"}
    try:
        import mjlab

        mjlab_import.update({"importOk": True, "version": getattr(mjlab, "__version__", "unknown")})
