...
class TestMotionSkillCompatibility:
    def test_g1_velocity_matches_g1_profile(self, motion_skill, g1_profile):
        report = check_compatibility(motion_skill, g1_profile)
    ...
    def test_undeclared_profile(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
        profile["metadata"]["id"] = "unitree.go2"
        ...
        assert "unitree.g1.29dof" in message

    def test_profile_version_out_of_range(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
        profile["metadata"]["version"] = "2.1.0"
        report = check_compatibility(motion_skill, profile)
        assert "compat.profile-version" in _error_rules(report)

    def test_joint_set_mismatch(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
        profile["description"]["jointSet"] = "g1.23dof.canonical.v1"
        report = check_compatibility(motion_skill, profile)
        assert "compat.joint-set" in _error_rules(report)

    def test_control_mode_mismatch(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
        profile["control"]["mode"] = "joint_torque"
        report = check_compatibility(motion_skill, profile)
        assert "compat.control-mode" in _error_rules(report)

    def test_control_hz_mismatch(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
        profile["control"]["frequencyHz"] = 200
        report = check_compatibility(motion_skill, profile)
        assert "compat.control-hz" in _error_rules(report)

    def test_simulation_target_required(self, motion_skill, g1_profile):
        profile = copy.deepcopy(g1_profile)
    ...
    def test_deploy_action_explains_physical_gate(self, motion_skill, g1_profile):
        report = check_compatibility(motion_skill, g1_profile)
...
class TestNonMotionSkill:
    def test_platform_skill_not_robot_bound(self, platform_skill, g1_profile):
        report = check_compatibility(platform_skill, g1_profile)
        assert report.ok
        assert "compat.not-robot-bound" in _rules(report)

    def test_agent_skill_not_robot_bound(self, agent_skill, g1_profile):
        report = check_compatibility(agent_skill, g1_profile)
        assert report.ok
