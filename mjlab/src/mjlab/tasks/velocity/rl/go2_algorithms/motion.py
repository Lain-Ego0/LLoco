"""AMP motion-trajectory loader for the migrated Go2 tasks.
...
def joint_permutation(
  ...
  if len(source) != len(target) or len(set(source)) != len(source):
    raise ValueError("source and target joint layouts must contain unique equal-sized names")
...
class Go2MotionLoader:
  ...
  def __init__(
    ...
    weights = torch.tensor([trajectory.weight for trajectory in self.trajectories], dtype=torch.float32)
  ...
  def sample_transition(
    ...
    step = self.time_between_frames if time_between_frames is None else float(time_between_frames)
