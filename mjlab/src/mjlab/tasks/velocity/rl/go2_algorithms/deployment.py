"""Deployment-side input adapters for exported Go2 conditional policies.
...
class Go2PolicyInputSpec:
  """Fixed dimensions shared by the source Go2 deployment contracts."""
...
class Go2OnnxPolicy:
  ...
  def act(self, actor: np.ndarray, conditional: np.ndarray) -> np.ndarray:
    ...
    if actor.ndim != 2 or conditional.ndim != 2 or actor.shape[0] != conditional.shape[0]:
      raise ValueError("actor and conditional must be batched arrays with equal batch size")
    ...
    if self.conditional_dim is not None and conditional.shape[-1] != self.conditional_dim:
      raise ValueError(f"conditional must have shape [batch, {self.conditional_dim}]")
    output = self.session.run(["actions"], {"actor": actor, "conditional": conditional})[0]
...
class Go2RecurrentOnnxPolicy:
  ...
  def __init__(
    ...
    if self.num_layers is not None and self.num_layers != Go2PolicyInputSpec.ts_student_lstm_layers:
      raise ValueError(f"Expected {Go2PolicyInputSpec.ts_student_lstm_layers} LSTM layers, got {self.num_layers}")
    if self.hidden_dim is not None and self.hidden_dim != Go2PolicyInputSpec.ts_student_hidden_dim:
      raise ValueError(
    ...
    if outputs[:3] != ["actions", "he", "ce"]:
      raise ValueError(
        "Expected recurrent ONNX outputs ['actions', 'he', 'ce'], "
        f"got {outputs}"
      )
  ...
  def act(
    ...
    if hidden.ndim != 3 or cell.shape != hidden.shape:
      raise ValueError("hidden and cell must be matching [layers, batch, hidden] arrays")
...
class Go2DeploymentAdapter:
  ...
  def conditional(
    ...
    if terrain.ndim != 2 or privileged.ndim != 2 or terrain.shape[0] != privileged.shape[0]:
      raise ValueError("terrain and privileged must be batched with equal batch size")
    if terrain.shape[-1] != self.spec.terrain_dim:
      raise ValueError(f"ts_teacher terrain must have shape [batch, {self.spec.terrain_dim}]")
  ...
  def act(
    ...
    return torch.from_numpy(
      self.policy.act(
        actor.detach().cpu().numpy(), conditional.detach().cpu().numpy()
      )
