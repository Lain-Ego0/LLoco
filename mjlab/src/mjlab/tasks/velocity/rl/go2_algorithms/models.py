"""PyTorch models for CTS, AMP, DreamWaQ and teacher-student Go2 policies."""
...
class CtsActorCritic(nn.Module):
  ...
  def act(
    ...
    latent = self.encode_teacher(privileged) if teacher else self.encode_student(history)
  ...
  def distillation_loss(self, privileged: torch.Tensor, history: torch.Tensor) -> torch.Tensor:
    return F.mse_loss(self.encode_student(history), self.encode_teacher(privileged).detach())
...
class DreamWaQVAE(nn.Module):
  ...
  def __init__(self, history_dim: int, latent_dim: int, explicit_dim: int, decode_dim: int):
    ...
    self.logvar_explicit = nn.Sequential(nn.Linear(64, explicit_dim), nn.Hardtanh(-5.0, 5.0))
...
class DreamWaQActorCritic(nn.Module):
  ...
  def __init__(
    ...
    self.policy = _GaussianPolicy(obs_dim + latent_dim + explicit_dim, action_dim, hidden_dims)
    self.value = _mlp(critic_dim, 1, hidden_dims)

  def act(self, obs: torch.Tensor, history: torch.Tensor, deterministic: bool = False) -> PolicyOutput:
...
class AmpDiscriminator(nn.Module):
  ...
  def forward(self, amp_obs: torch.Tensor, next_amp_obs: torch.Tensor | None = None) -> torch.Tensor:
  ...
  def loss(
    ...
    return 0.5 * (expert_loss + policy_loss) + gradient_penalty_weight * gradient_penalty
  ...
  def reward(
    ...
    with torch.no_grad():
      ...
      amp_reward = 0.02 * self.amp_reward_coef * torch.clamp(
...
class TeacherStudentActorCritic(nn.Module):
  ...
  def teacher_features(self, terrain: torch.Tensor, privileged: torch.Tensor) -> torch.Tensor:
    return torch.cat((self.terrain_encoder(terrain), self.privileged_encoder(privileged)), dim=-1)
  ...
  def student_features(self, history: torch.Tensor) -> torch.Tensor:
    if history.ndim == 2:
      if history.shape[-1] == self.obs_dim:
        ...
        raise ValueError(f"History dimension {history.shape[-1]} is not divisible by obs_dim {self.obs_dim}")
    sequence, _ = self.student_lstm(history)
    return self.student_head(sequence[:, -1])

  def act(self, obs: torch.Tensor, features: torch.Tensor, deterministic: bool = False) -> PolicyOutput:
    return self.policy.sample(torch.cat((obs, features), dim=-1), deterministic)

  def evaluate(self, critic_obs: torch.Tensor) -> torch.Tensor:
    return self.value(critic_obs)

  def distillation_loss(self, terrain: torch.Tensor, privileged: torch.Tensor, history: torch.Tensor) -> torch.Tensor:
    return F.mse_loss(self.student_features(history), self.teacher_features(terrain, privileged).detach())
...
class _Go2ConditionalActor(MLPModel):
  ...
  def __init__(self, obs, obs_groups, obs_set, output_dim, **kwargs) -> None:
    ...
    self.mlp = MLP(actor_dim + latent_dim, self.distribution.input_dim if self.distribution else output_dim, hidden_dims, activation)
  ...
  def _conditional_features(self, obs: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    if self.latent_kind == "ts":
      ...
      return torch.cat((self.terrain_encoder(obs["terrain"]), self.privileged_encoder(obs["privileged"])), dim=-1)
  ...
  def get_latent(self, obs, masks=None, hidden_state=None):
    ...
    return torch.cat((conditional, actor_obs), dim=-1) if conditional.shape[-1] else actor_obs
...
class _Go2ConditionalOnnxModel(nn.Module):
  ...
  def __init__(self, model: _Go2ConditionalActor, verbose: bool) -> None:
    ...
    for name in (
      "encoder", "teacher_encoder", "student_encoder", "vae", "student_lstm",
      "student_head", "terrain_encoder", "privileged_encoder",
    ):
  ...
  def _features(self, conditional: torch.Tensor) -> torch.Tensor:
    ...
    if self.latent_kind == "ts":
      ...
      terrain, privileged = torch.split(conditional, (terrain_dim, privileged_dim), dim=-1)
      return torch.cat((self.terrain_encoder(terrain), self.privileged_encoder(privileged)), dim=-1)
